...
(()=>{
 ...
 async function hydrateProfile(){
  if(readJSON('cb_profile'))return;const user=await currentUser();if(!user)return;const metadata=user.user_metadata||{},name=metadata.display_name||metadata.full_name||metadata.name||(user.email?user.email.split('@')[0]:'Player');localStorage.setItem('cb_profile',JSON.stringify({name:String(name).slice(0,24),avatar:window.CandyProfiles?.get(metadata.avatar)?.id||'berry'}));
 }
 ...
 function updateProfileButton(){
  ...
  if(homeName)homeName.textContent=name;if(homeStatus)homeStatus.textContent=session?`Level ${current} · ${stars} stars · Cloud`:`Level ${current} · Guest journey`;window.CandyProfiles?.mount(avatar,profile?.avatar,{label:false});
 }
 async function accountScreen(){
  ...
  const cloudError=localStorage.getItem('cb_cloud_error'),cloudText=cloudError?'Database setup required':navigator.onLine?(last?'Synced '+new Date(last).toLocaleString():'Ready to sync'):'Offline · changes stay on this device',map=readJSON('cb_map',{}),current=Math.max(1,+map?.currentLevel||1),stars=(map?.levels||[]).reduce((sum,item)=>sum+(+item.stars||0),0),best=+localStorage.getItem('cb_best')||0,name=profile?.name||user?.user_metadata?.display_name||user?.user_metadata?.full_name||user?.user_metadata?.name||'Player',wallet=window.CandyEconomy?.state||{},avatar=window.CandyProfiles?.get(profile?.avatar);
  const avatarButtons=(window.CandyProfiles?.items||[]).map(item=>`<button type="button" class="avatar-choice${item.id===avatar?.id?' active':''}" data-avatar="${item.id}" aria-label="${item.name}" aria-pressed="${item.id===avatar?.id}"><img src="${item.src}" alt=""></button>`).join('');
