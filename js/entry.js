...
(()=>{
 ...
 function mergeSnapshots(local,remote){
  ...
  if(local.cb_tutorial_done||remote.cb_tutorial_done)base.cb_tutorial_done='1';
 ...
 function shell(content,label='Candy Blast account'){
  document.querySelectorAll('.screen').forEach(el=>el.inert=true);let overlay=document.getElementById('entry-overlay');if(!overlay){overlay=document.createElement('div');overlay.id='entry-overlay';overlay.className='entry-overlay';document.body.append(overlay);}overlay.setAttribute('role','dialog');overlay.setAttribute('aria-modal','true');overlay.setAttribute('aria-label',label);overlay.innerHTML=`<div class="entry-card"><img src="images/ui/candyblast-emblem.webp" alt=""><div id="entry-content">${content}</div><p id="entry-error" role="status" aria-live="polite"></p></div>`;return overlay;
 }
 function authScreen(mode='signin'){
  const googleMark='<span class="provider-mark google-mark" aria-hidden="true"><svg viewBox="0 0 24 24"><path fill="#4285F4" d="M21.6 12.23c0-.71-.06-1.39-.18-2.05H12v3.87h5.38a4.6 4.6 0 0 1-2 3.02v2.51h3.24c1.89-1.74 2.98-4.31 2.98-7.35Z"/><path fill="#34A853" d="M12 22c2.7 0 4.96-.9 6.62-2.42l-3.24-2.51c-.89.6-2.04.95-3.38.95-2.61 0-4.81-1.76-5.6-4.12H3.06v2.59A10 10 0 0 0 12 22Z"/><path fill="#FBBC05" d="M6.4 13.9A6 6 0 0 1 6.09 12c0-.66.11-1.3.31-1.9V7.51H3.06A10 10 0 0 0 2 12c0 1.61.39 3.14 1.06 4.49L6.4 13.9Z"/><path fill="#EA4335" d="M12 5.98c1.47 0 2.79.5 3.82 1.49l2.87-2.87C16.96 2.99 14.7 2 12 2a10 10 0 0 0-8.94 5.51L6.4 10.1c.79-2.36 2.99-4.12 5.6-4.12Z"/></svg></span>';
 ...
 function updateProfileButton(){
  ...
  const button=document.querySelector('.profile-menu');if(button)button.textContent=session?`${name} · Cloud account`:'Player account';
  const homeName=document.getElementById('home-profile-name'),homeStatus=document.getElementById('home-profile-status'),avatar=document.getElementById('home-profile-avatar');
  if(homeName)homeName.textContent=name;if(homeStatus)homeStatus.textContent=session?`Level ${current} · ${stars} stars · Cloud`:`Level ${current} · Guest journey`;if(avatar)avatar.textContent=name.slice(0,1).toUpperCase();
 }
 async function accountScreen(){
  ...
  const cloudError=localStorage.getItem('cb_cloud_error'),cloudText=cloudError?'Database setup required':navigator.onLine?(last?'Synced '+new Date(last).toLocaleString():'Ready to sync'):'Offline · changes stay on this device',map=readJSON('cb_map',{}),current=Math.max(1,+map?.currentLevel||1),stars=(map?.levels||[]).reduce((sum,item)=>sum+(+item.stars||0),0),best=+localStorage.getItem('cb_best')||0,name=profile?.name||user?.user_metadata?.display_name||user?.user_metadata?.full_name||user?.user_metadata?.name||'Player';
  const overlay=shell(`<div class="account-hero"><span class="account-avatar"></span><div><h1>Your journey</h1><p class="account-email"></p></div></div><div class="account-stats"><div><strong>${current}</strong><small>Level</small></div><div><strong>${stars}</strong><small>Stars</small></div><div><strong>${best.toLocaleString()}</strong><small>Best</small></div></div><div class="cloud-state${cloudError?' has-error':''}"><span class="cloud-dot"></span><div><strong>Cloud progress</strong><small>${cloudText}</small></div></div><form id="profile-form"><label for="player-name">Player name</label><input id="player-name" autocomplete="nickname" maxlength="24" required><button class="btn btn-play" type="submit">Save & sync</button></form><button class="btn btn-secondary" id="account-close" type="button">Back to game</button><button class="auth-link danger-link" id="account-signout" type="button">Sign out</button>`,'Player account');overlay.querySelector('.account-avatar').textContent=name.slice(0,1).toUpperCase();overlay.querySelector('.account-email').textContent=user?.email||'';overlay.querySelector('#player-name').value=name;overlay.querySelector('#account-close').onclick=leave;overlay.querySelector('#account-signout').onclick=signOut;overlay.querySelector('form').onsubmit=async e=>{e.preventDefault();const name=overlay.querySelector('#player-name').value.trim();if(!name)return;localStorage.setItem('cb_profile',JSON.stringify({name}));setBusy(true);try{await request('/auth/v1/user',{method:'PUT',body:{data:{display_name:name}}});await pushCloud();setStatus('Profile and progress synced.');updateProfileButton();overlay.querySelector('.account-avatar').textContent=name.slice(0,1).toUpperCase();}catch(error){setStatus(message(error),true);}finally{setBusy(false);}};
 }
