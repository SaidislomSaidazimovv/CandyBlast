...
(() => {
  ...
  function replace(node){
    ...
    for(const match of text.matchAll(pattern)){
      ...
      if(match[0]==='🖤')span.classList.add('icon-empty');
      const svg=document.createElementNS('http://www.w3.org/2000/svg','svg'),use=document.createElementNS(svg.namespaceURI,'use');
      svg.setAttribute('aria-hidden','true');svg.setAttribute('viewBox','0 0 24 24');use.setAttribute('href','images/ui/icons.svg#'+name);svg.append(use);span.append(svg);fragment.append(span);cursor=match.index+match[0].length;
    }
