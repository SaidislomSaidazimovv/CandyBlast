...
window.Candy3DBridge={
  snapshot(){return {cells:Array.from({length:64},(_,i)=>({type:grid?.[Math.floor(i/8)]?getType(Math.floor(i/8),i%8):-1,special:grid?.[Math.floor(i/8)]?getSpecial(Math.floor(i/8),i%8):null,ice:objective.ice.includes(i),iceHits:objective.iceHits?.[i]||0})),session:gameSession,active:currentScreen==='game',paused:paused||pregame||gameEnded,anim:settings.anim,flashes:settings.flashes};},
