...
test('mobile launch defers 3D and native shells use the OAuth deep link',()=>{
  ...
  assert.doesNotMatch(html,/type="module" src="js\/renderer3d\.mjs"/);assert.match(loader,/import\('\.\/renderer3d\.mjs'\)/);
  assert.doesNotMatch(sw,/vendor\/three/);assert.match(entry,/candyblast:\/\/auth\/callback/);
...
test('launch levels have deterministic authored world scenes',()=>{const source=fs.readFileSync(path.join(root,'js/backgrounds.js'),'utf8'),sw=fs.readFileSync(path.join(root,'sw.js'),'utf8');assert.match(source,/Berry Meadow/);assert.match(source,/Sundae Harbour/);assert.match(source,/Mintwood Grove/);assert.match(source,/Caramel Peaks/);assert.match(source,/variantFor/);assert.match(source,/Math\.floor\(\(clampLevel\(level\) - 1\) \/ 5\)/);assert.match(source,/--app-plum/);assert.match(sw,/url\.origin === self\.location\.origin/);});
test('brand assets and unboxed action icons ship offline',()=>{const html=fs.readFileSync(path.join(root,'index.html'),'utf8'),css=fs.readFileSync(path.join(root,'css/worlds.css'),'utf8'),sw=fs.readFileSync(path.join(root,'sw.js'),'utf8');assert.match(html,/candyblast-hero\.webp/);assert.match(html,/app-icon-512\.png/);assert.match(css,/#candy-world \.world-sun[^}]+display:none!important/);assert.match(css,/\.home-nav button>\.ui-icon[^}]+background:none!important/);assert.match(css,/\.ingame-booster-btn\{background:transparent!important;border:0!important/);for(const asset of ['app-icon-512.png','candyblast-hero.webp','candyblast-emblem.webp'])assert.match(sw,new RegExp(asset.replace('.','\\.')));});
test('official provider marks and live leaderboard wiring are present',()=>{const entry=fs.readFileSync(path.join(root,'js/entry.js'),'utf8'),leaderboard=fs.readFileSync(path.join(root,'js/leaderboard.js'),'utf8'),sql=fs.readFileSync(path.join(root,'supabase/migrations/002_live_leaderboard.sql'),'utf8'),html=fs.readFileSync(path.join(root,'index.html'),'utf8');assert.match(entry,/fill="#4285F4"/);assert.match(entry,/appleMark=.*<svg/);assert.match(html,/js\/leaderboard\.js/);assert.match(leaderboard,/postgres_changes/);assert.match(leaderboard,/setInterval\(refresh,15000\)/);assert.match(sql,/alter publication supabase_realtime add table public\.leaderboard_scores/);});
test('an invalid swap restores the board without costing a move',async()=>{const g=game();g.run('startMapLevel(4);hidePreGame();settings.anim=false;delay=async()=>{};var invalidPair=null;outer:for(let r=0;r<GRID;r++)for(let c=0;c<GRID;c++)for(const [dr,dc] of [[0,1],[1,0]]){const rr=r+dr,cc=c+dc;if(rr>=GRID||cc>=GRID)continue;[grid[r][c],grid[rr][cc]]=[grid[rr][cc],grid[r][c]];const valid=findMatchesNew().matched.length>0;[grid[r][c],grid[rr][cc]]=[grid[rr][cc],grid[r][c]];if(!valid){invalidPair=[r,c,rr,cc];break outer;}}var beforeMoves=moves;var beforeBoard=JSON.stringify(grid);');assert.ok(g.run('invalidPair'));await g.run('trySwap(...invalidPair)');assert.equal(g.run('moves'),g.run('beforeMoves'));assert.equal(g.run('JSON.stringify(grid)'),g.run('beforeBoard'));});

test('loaded lives and booster inventory are normalized to safe bounds',()=>{const g=game();g.storage.set('cb_lives',JSON.stringify({lives:88,lastLostAt:-50,boosters:{extraMoves:-7,hammer:4.9,bomb:99999}}));g.run('loadLives();');assert.equal(g.run('livesData.lives'),5);assert.equal(g.run('livesData.lastLostAt'),null);assert.equal(g.run('livesData.boosters.extraMoves'),0);assert.equal(g.run('livesData.boosters.hammer'),4);assert.equal(g.run('livesData.boosters.bomb'),999);});

test('a future life timestamp is clamped and cannot grant free lives',()=>{const g=game();g.run('livesData={lives:2,lastLostAt:Date.now()+LIFE_REGEN_MS*4,boosters:{extraMoves:0,hammer:0,bomb:0}};saveLives();regenLives();');assert.equal(g.run('livesData.lives'),2);assert.ok(g.run('getTimeUntilNextLife()')>0);});

test('a failed level loses exactly one life even when result checks repeat',()=>{const g=game();g.run('startMapLevel(2);hidePreGame();moves=0;finishMove();finishMove();checkEnd();');assert.equal(g.run('livesData.lives'),4);});

test('winning on the last move does not consume a life',()=>{const g=game();g.run('startMapLevel(1);hidePreGame();moves=0;score=targetScore;levelScore=targetScore;finishMove();');assert.equal(g.run('gameEnded'),true);assert.equal(g.run('livesData.lives'),5);});

test('targeted boosters wait for a target and never spend a move',async()=>{const g=game();g.run('startMapLevel(2);hidePreGame();settings.anim=false;var initialMoves=moves;var hammerStock=livesData.boosters.hammer;useIngameBooster("hammer");');assert.equal(g.run('livesData.boosters.hammer'),g.run('hammerStock-1'));assert.equal(g.run('moves'),g.run('initialMoves'));g.run('getCell=()=>({classList:{add(){},remove(){}}});onCellClick({currentTarget:{dataset:{r:"0",c:"0"}}});');await g.pending.at(-1)();assert.equal(g.run('moves'),g.run('initialMoves'));assert.equal(g.run('livesData.boosters.hammer'),g.run('hammerStock-1'));});

test('gameplay feedback assets are loaded offline and HUD announces authored level names',()=>{const html=fs.readFileSync(path.join(root,'index.html'),'utf8'),sw=fs.readFileSync(path.join(root,'sw.js'),'utf8'),css=fs.readFileSync(path.join(root,'css/gameplay-polish.css'),'utf8');assert.match(html,/css\/gameplay-polish\.css/);assert.match(html,/id="game-level-name"/);assert.match(html,/id="combo-display" role="status" aria-live="polite"/);assert.match(sw,/css\/gameplay-polish\.css/);assert.match(css,/prefers-reduced-motion/);});

test('gameplay sound chain includes compressed output and distinct booster feedback',()=>{const source=fs.readFileSync(path.join(root,'js/game.js'),'utf8');assert.match(source,/createDynamicsCompressor/);assert.match(source,/function playNoise/);assert.match(source,/function playBoosterSound/);assert.match(source,/function playLowMoves/);});

test('double frost takes two separate hits and reports remaining layers',()=>{const g=game();g.run('startMapLevel(9);var frostTile=objective.ice[0],frostRow=Math.floor(frostTile/GRID),frostCol=frostTile%GRID,initialIce=objective.ice.length;grid[frostRow][frostCol]=0;removeCandy(frostRow,frostCol);');assert.equal(g.run('objective.ice.length'),g.run('initialIce'));assert.equal(g.run('objective.iceHits[frostTile]'),1);g.run('grid[frostRow][frostCol]=1;removeCandy(frostRow,frostCol);');assert.equal(g.run('objective.ice.length'),g.run('initialIce-1'));assert.equal(g.run('objective.iceHits[frostTile]'),undefined);});

test('frost layers survive a saved-game round trip',()=>{const g=game();g.run('startMapLevel(11);hidePreGame();var tile=objective.ice[0],row=Math.floor(tile/GRID),col=tile%GRID;grid[row][col]=0;removeCandy(row,col);saveGameState();restoreGameState();');assert.equal(g.run('objective.ice.includes(tile)'),true);assert.equal(g.run('objective.iceHits[tile]'),1);});

test('the 20-level curve introduces double frost only after onboarding',()=>{const g=game();const levels=JSON.parse(g.run('JSON.stringify(generateLevels())'));assert.equal(levels.length,20);assert.equal(levels.slice(0,8).every(level=>level.objective.iceLayers===1),true);for(const id of [9,11,12,15,17,19,20])assert.equal(levels[id-1].objective.iceLayers,2);assert.ok(levels[0].targetScore<levels[7].targetScore);assert.ok(levels[7].targetScore<=levels[15].targetScore);});

test('journey is divided into four five-level worlds with milestone gates',()=>{const g=game();assert.equal(g.run('REGIONS.length'),4);assert.deepEqual(JSON.parse(g.run('JSON.stringify(REGIONS.map(r=>r.levels))')),[[1,5],[6,10],[11,15],[16,20]]);assert.equal(g.run('getRegionForLevel(1).id'),'berry-meadow');assert.equal(g.run('getRegionForLevel(20).id'),'caramel-peaks');const source=fs.readFileSync(path.join(root,'js/map.js'),'utf8'),css=fs.readFileSync(path.join(root,'css/journey.css'),'utf8'),html=fs.readFileSync(path.join(root,'index.html'),'utf8'),sw=fs.readFileSync(path.join(root,'sw.js'),'utf8');assert.match(source,/journey-chapter/);assert.match(source,/chapter-gate/);assert.match(css,/--chapter-image/);assert.match(html,/css\/journey\.css/);assert.match(sw,/css\/journey\.css/);});
test('chapter stars award bronze, silver and gold mastery without spending progress',()=>{const g=game();g.run('mapData.levels=generateLevels();');assert.equal(g.run('getRegionMastery(REGIONS[0]).tier'),'');g.run('mapData.levels[0].stars=3;mapData.levels[1].stars=2;');assert.equal(g.run('getRegionMastery(REGIONS[0]).tier'),'Bronze');g.run('mapData.levels[2].stars=3;mapData.levels[3].stars=2;');assert.equal(g.run('getRegionMastery(REGIONS[0]).tier'),'Silver');g.run('mapData.levels[3].stars=3;mapData.levels[4].stars=2;');assert.equal(g.run('getRegionMastery(REGIONS[0]).tier'),'Silver');g.run('mapData.levels[1].stars=3;mapData.levels[4].stars=3;');assert.equal(g.run('getRegionMastery(REGIONS[0]).tier'),'Gold');assert.equal(g.run('getTotalStars()'),15);});
test('the journey reveal targets only a newly unlocked level',()=>{const g=game();g.run('completeLevel(1,2,4000);');assert.equal(g.run('journeyReveal.from'),1);assert.equal(g.run('journeyReveal.to'),2);assert.equal(g.run('mapData.levels[1].locked'),false);g.run('journeyReveal=null;completeLevel(1,3,6000);');assert.equal(g.run('journeyReveal'),null);assert.equal(g.run('mapData.currentLevel'),2);assert.match(g.run('journeyPath([{x:20,y:0},{x:80,y:132}],0,1)'),/^M20 0 C/);});

test('authenticated economy is isolated from client-authored cloud progress',()=>{const entry=fs.readFileSync(path.join(root,'js/entry.js'),'utf8'),economy=fs.readFileSync(path.join(root,'js/economy.js'),'utf8'),html=fs.readFileSync(path.join(root,'index.html'),'utf8'),sw=fs.readFileSync(path.join(root,'sw.js'),'utf8');const tracked=entry.match(/const TRACKED_KEYS=\[([^\]]+)\]/)?.[1]||'';for(const key of ['cb_lives','cb_daily','cb_spin_given'])assert.doesNotMatch(tracked,new RegExp(key));assert.match(entry,/rpc:\(name,body=\{\}\)/);assert.match(economy,/economy_get_state/);assert.match(economy,/economy_consume/);assert.match(economy,/economy_claim_daily/);assert.match(economy,/economy_spin/);assert.match(html,/js\/economy\.js/);assert.match(sw,/js\/economy\.js/);});

test('server economy migration uses authenticated atomic ledgers',()=>{const sql=fs.readFileSync(path.join(root,'supabase/migrations/003_server_economy.sql'),'utf8');assert.match(sql,/create table if not exists public\.player_wallets/);assert.match(sql,/unique \(user_id, claim_key\)/);assert.match(sql,/auth\.uid\(\)/);assert.match(sql,/security definer/);assert.match(sql,/for update/);assert.match(sql,/economy_claim_daily/);assert.match(sql,/economy_claim_weekly_bonus/);assert.match(sql,/economy_spin/);assert.match(sql,/random\(\)/);assert.match(sql,/revoke all on function/);assert.match(sql,/grant execute on function/);});
