...
test('brand assets and unboxed action icons ship offline',()=>{const html=fs.readFileSync(path.join(root,'index.html'),'utf8'),css=fs.readFileSync(path.join(root,'css/worlds.css'),'utf8'),sw=fs.readFileSync(path.join(root,'sw.js'),'utf8');assert.match(html,/candyblast-logo\.webp/);assert.match(html,/app-icon-512\.png/);assert.match(css,/#candy-world \.world-sun[^}]+display:none!important/);assert.match(css,/\.home-nav button>\.ui-icon[^}]+background:none!important/);assert.match(css,/\.ingame-booster-btn\{background:transparent!important;border:0!important/);for(const asset of ['app-icon-512.png','candyblast-logo.webp','candyblast-emblem.webp'])assert.match(sw,new RegExp(asset.replace('.','\\.')));});
