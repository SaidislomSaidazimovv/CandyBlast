...
for(let winner=0;winner<9;winner++)test('spin ends on exactly winner '+winner+' and claims once',()=>{
 ...
 const button={style:{}},nodes={'spin-grid':{style:{}},'spin-action-btn':button,'spin-remaining':{}};slots.forEach((s,i)=>nodes['spin-slot-'+i]=s);let rewards=0,ticks=0;
 ...
 vm.runInContext(fs.readFileSync('js/spin.js','utf8'),ctx);vm.runInContext('doSpin('+winner+');doSpin('+winner+');',ctx);assert.equal(ctx.dailyData.spinCount,0);assert.equal(nodes['spin-grid'].style.transform,'rotate('+(1800-winner*40)+'deg)');
...
test('signed-in spin animates the winner returned by the server',async()=>{
 const timers=[],slots=Array.from({length:9},()=>({style:{},classes:new Set(),classList:{remove(n){this.owner.classes.delete(n)},toggle(n,on){on?this.owner.classes.add(n):this.owner.classes.delete(n)}}}));slots.forEach(s=>s.classList.owner=s);
 const button={style:{}},nodes={'spin-grid':{style:{}},'spin-action-btn':button,'spin-remaining':{}};slots.forEach((s,i)=>nodes['spin-slot-'+i]=s);let localRewards=0;
 ...
 vm.runInContext(fs.readFileSync('js/spin.js','utf8'),ctx);await vm.runInContext('doSpin(0)',ctx);assert.equal(nodes['spin-grid'].style.transform,'rotate(1520deg)');while(timers.length)timers.shift()();assert.deepEqual(slots.map((s,i)=>s.classes.has('is-winner')?i:-1).filter(i=>i>=0),[7]);button.onclick();assert.equal(localRewards,0);
});
